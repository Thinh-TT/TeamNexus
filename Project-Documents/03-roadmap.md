...
**Yêu cầu hoàn thiện:**
- [ ] Project backend (Modular Monolith) và frontend (React/Vite) khởi tạo, build/run được
- [ ] Schema PostgreSQL cơ bản (User, Role, Workspace/Board) đã migrate
- [ ] Đăng nhập được qua Google và GitHub OAuth
- [ ] JWT + refresh token qua HttpOnly Cookie hoạt động, có rotate refresh token
- [ ] Phân quyền Policy-based cho 3 role (Admin/Manager/Member) áp dụng được trên ít nhất 1 endpoint mẫu
...
**Yêu cầu hoàn thiện:**
- [ ] Bảng `AiActionLog` lưu action, basis (tóm tắt input), timestamp, actor, trạng thái
- [ ] Service `AiActionService` là điểm duy nhất mọi hành động AI đi qua trước khi ghi dữ liệu thật
- [ ] Smart Setup (giai đoạn 3) đã chuyển sang dùng luồng Pending → Approve/Reject qua service này
- [ ] Chức năng Undo hoạt động cho ít nhất 1 loại hành động AI (vd: tạo hàng loạt sub-tasks)
