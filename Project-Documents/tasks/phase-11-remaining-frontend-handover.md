...
| **B3b — antiforgery token gắn với danh tính** | Token CSRF cache **gắn với người đăng nhập đã lấy nó**: đăng nhập A → B → A rồi POST bằng token cũ ⇒ **403**, đọc rất dễ nhầm thành "service chặn sai". Dùng `scenario.PostWithFreshAntiforgeryAsync(client, url, content?)` khi có ≥ 2 lần đăng nhập trong 1 test | **Có, gián tiếp** — `httpClient.ts` (production) đã có đúng cơ chế retry khi gặp CSRF 403; **đừng** viết thêm retry trong `memberApi`/`profileApi` |
